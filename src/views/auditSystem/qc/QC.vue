    ...
    <div class="DAR-container">
        ...
        <el-container>
            ...
            <el-main class="DAR-main-container">
                <el-table :data="QCAudit.arr" style="width: 100%" border>
                    <el-table-column prop="c_name" label="企业名称" width="180" />
                    <el-table-column prop="c_unified_social_credit_code" label="社会信用代码" />
                    <el-table-column prop="l_name" label="联系人姓名" />
                    <!-- <el-table-column prop="renew_time" label="数据更新实践" />
                    <el-table-column prop="audit_status" label="审核状态" /> -->

                    <el-table-column label="操作"> 
                        <template #default="scope">
                            <el-button size="small" @click="toDetail(scope.row)">详情信息</el-button>
                        </template>
                    </el-table-column>
                </el-table>
...
import { getQCAudit, getQCAuditCount,renewQCAudit } from '@/api/user'
...
//data
let QCAudit = reactive<{ arr: string[] }>({ arr: [] })

getQCAuditCount().then(res => {
    if (res.status == 200) {
...
getQCAudit(page.current_page).then(res => {
    if (res.status == 200) {
        if (res.data.data != null) {
            QCAudit.arr = res.data.data
        }
    }
})
